...
import {
  getFondosEscalamientoConfig,
  setFondoEscalamientoEnabled,
  type FondoEscalamientoConfig,
} from '@/lib/actions/escalamiento';
import { TrendingUp } from 'lucide-react';
import { usePageTopLoader } from '@/hooks/usePageTopLoader';

export default function ConfiguracionEscalamientoPage() {
  const [fondos, setFondos] = useState<FondoEscalamientoConfig[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    const res = await getFondosEscalamientoConfig();
    if (!res.success) {
  ...
  const handleToggle = async (
    ...
    setFondos((prev) =>
      prev.map((f) =>
        f.id === fondo.id ? { ...f, escalamientoEnabled: enabled } : f
      )
    );
    const res = await setFondoEscalamientoEnabled(fondo.id, enabled);
    if (!res.success) {
      ...
      <div className="max-w-3xl space-y-6">
        <div>
          ...
          <p className="mt-1 text-[13px] text-gray-500 leading-relaxed">
            Solo los proyectos de fondos en On verán el tab Escalamiento con el
            documento de sistematización y expansión.
          </p>
        </div>
        ...
        <div className="flex items-start gap-3 rounded-md border border-gray-200 bg-gray-50/70 px-4 py-3">
          ...
          <p className="text-[13px] text-gray-600 leading-relaxed">
            El contenido se guarda por proyecto. Incluye nuevas instancias
            potenciales e acuerdos o compromisos de continuidad o expansión.
          </p>
        </div>
                  ...
                  <TableRow key={fondo.id} className="hover:bg-gray-50/50">
                    ...
                    <TableCell className="text-right">
                      <div className="inline-flex items-center gap-2 justify-end">
                        <span
                          className={`text-[12px] ${
                            fondo.escalamientoEnabled
                              ? 'text-emerald-700'
                              : 'text-gray-400'
                          }`}
                        >
                          {fondo.escalamientoEnabled ? 'On' : 'Off'}
                        </span>
                        <Switch
                          checked={fondo.escalamientoEnabled}
                          disabled={togglingId === fondo.id}
                          onCheckedChange={(checked) =>
                            handleToggle(fondo, checked)
                          }
                          aria-label={`Escalamiento para ${fondo.nombre}`}
                        />
                      </div>
                    </TableCell>
                  </TableRow>
