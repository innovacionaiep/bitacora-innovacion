...
function filterAndSortEventos(posts: PostWithRelations[]): PostWithRelations[] {
  return posts
    ...
    .sort((a, b) => {
      ...
      return (
        new Date(a.eventoFecha).getTime() - new Date(b.eventoFecha).getTime()
      );
    });
}

// Agrupar eventos por fecha (YYYY-MM-DD)
function groupEventosByDate(
  eventos: PostWithRelations[]
...
export function EventosWall({
  ...
  const [eventos, setEventos] = useState<PostWithRelations[]>(() =>
    filterAndSortEventos(initialEventos)
  );
    ...
    setEventos((cur) =>
      cur.map((e) => {
        ...
        const nextCount = Math.max(
          0,
          (e.asistentesCount ?? 0) + (nextIsAsistiendo ? 1 : -1)
        );
        return {
          ...e,
          isAsistiendo: nextIsAsistiendo,
          asistentesCount: nextCount,
        };
      })
    );
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          <div className="flex items-center gap-1">
                            <Button
                              type="button"
                              variant={
                                evento.isAsistiendo ? 'default' : 'outline'
                              }
                              size="sm"
                          ...
                          >
                            <div className="space-y-2.5 p-1 bg-white rounded-lg border border-gray-200 shadow-lg">
                              {dayEventos.map((evento) => {
                                if (!evento.eventoFecha || !evento.eventoNombre)
                                  return null;
                                return (
                                  <Card
                                    key={evento.id}
