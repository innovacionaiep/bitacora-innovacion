...
export type ImpulsaExcelSettingsView = {
  filePath: string;
  sheetName: string;
...
function toView(stored: PortalAvancesImpulsaStored): ImpulsaExcelSettingsView {
  return {
    filePath: stored.filePath || PORTAL_AVANCES_IMPULSA_DEFAULT_PATH,
    sheetName: stored.sheetName || PORTAL_AVANCES_IMPULSA_DEFAULT_SHEET,
...
export async function getImpulsaExcelSettings(): Promise<{
  success: boolean;
  data?: ImpulsaExcelSettingsView;
  error?: string;
}> {
  const gate = await requireAdmin();
  if (!gate.ok) return { success: false, error: gate.error };
  const stored = await readImpulsaStored();
  return { success: true, data: toView(stored) };
}

export async function saveImpulsaExcelSettings(input: {
  filePath: string;
  sheetName: string;
}): Promise<{ success: boolean; data?: ImpulsaExcelSettingsView; error?: string }> {
  const gate = await requireAdmin();
  if (!gate.ok) return { success: false, error: gate.error };
  const stored = await readImpulsaStored();
  const filePath = input.filePath.trim() || PORTAL_AVANCES_IMPULSA_DEFAULT_PATH;
  const sheetName =
    input.sheetName.trim() || PORTAL_AVANCES_IMPULSA_DEFAULT_SHEET;
  ...
  await writeImpulsaStored(next);
  return { success: true, data: toView(next) };
}

export async function testImpulsaExcelFile(): Promise<{
  success: boolean;
  error?: string;
}> {
  const gate = await requireAdmin();
  ...
  await writeImpulsaStored({ ...stored, fileOk: true });
  return { success: true };
}

export async function testImpulsaExcelSheet(): Promise<{
  success: boolean;
  error?: string;
}> {
  const gate = await requireAdmin();
  ...
  await writeImpulsaStored({ ...stored, fileOk: true, sheetOk: true });
  return { success: true };
}

export async function updateImpulsaExcelSnapshot(): Promise<{
  success: boolean;
  data?: ImpulsaExcelSettingsView;
  error?: string;
}> {
