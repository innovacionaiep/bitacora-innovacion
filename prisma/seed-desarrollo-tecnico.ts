...
async function seedDesarrolloTecnico() {
  ...
  try {
    ...
    console.log(`📊 Encontrados ${proyectos.length} proyectos para poblar con desarrollo técnico`);
    ...
    for (const proyecto of proyectos) {
      ...
      const desarrolloTecnico = await prisma.desarrolloTecnico.upsert({
        ...
        create: {
          ...
          continuidadFasesAnteriores: getContinuidadFasesAnteriores(proyecto.proyecto),
          pertinenciaLocal: getPertinenciaLocal(proyecto.sede, proyecto.focalizacion),
          pertinenciaDisciplinar: getPertinenciaDisciplinar(proyecto.proyecto, proyecto.sede),
          necesidadProblema: getNecesidadProblema(proyecto.proyecto, proyecto.sede),
          publicoObjetivo: getPublicoObjetivo(proyecto.proyecto, proyecto.sede),
          solucionAvance: getSolucionAvance(proyecto.proyecto, proyecto.avanceGantt),
...
function getContinuidadFasesAnteriores(nombreProyecto: string): string {
  const continuidades: Record<string, string> = {
    'TechLakou': 'Este proyecto es una iniciativa pionera en la alfabetización digital para la comunidad haitiana en Chile. No es continuidad de fases anteriores, sino una propuesta innovadora que surge de la identificación de brechas digitales específicas en la población migrante. Los elementos diferenciadores incluyen el enfoque intercultural, la adaptación de herramientas digitales al contexto haitiano, y la integración de competencias lingüísticas funcionales.',
    'AntofaSuena': 'AntofaSuena representa la evolución natural de iniciativas musicales previas en la región. Se basa en el aprendizaje de festivales anteriores organizados por la comunidad artística local. Los elementos innovadores incluyen la integración de tecnología digital para la promoción, la creación de una plataforma online permanente, y el enfoque en la sostenibilidad económica del sector musical regional.',
    'Laboratorio de Innovación Gastronómico': 'No aplica. Este es un proyecto nuevo que surge de la necesidad de modernizar la gastronomía regional con técnicas contemporáneas.',
    'Aqua Terra': 'No aplica. Proyecto inaugural que combina estética consciente con sostenibilidad ambiental.',
  };

  return continuidades[nombreProyecto] || 'No aplica. Este proyecto representa una nueva iniciativa en el área correspondiente.';
}

function getPertinenciaLocal(sede: string, focalizacion: string | null): string {
  const pertinencias: Record<string, string> = {
    'San Bernardo': 'San Bernardo presenta una alta concentración de población migrante, especialmente de origen haitiano, que enfrenta barreras de integración socioeconómica. El proyecto es altamente pertinente para abordar las brechas digitales y de emprendimiento que limitan las oportunidades de desarrollo de esta comunidad. La localización estratégica permite un impacto directo en la población objetivo.',
    'Antofagasta': 'Antofagasta, como capital minera del país, presenta una economía tradicional que requiere diversificación. El proyecto es pertinente para desarrollar nuevas industrias creativas y tecnológicas que complementen la actividad minera, generando empleo y oportunidades para jóvenes profesionales.',
    'La Serena': 'La Serena, con su patrimonio histórico y potencial turístico, requiere iniciativas que fortalezcan su oferta cultural y gastronómica. El proyecto es pertinente para posicionar la región como destino de turismo cultural y gastronómico.',
    'Los Ángeles': 'Los Ángeles, en la región del Biobío, presenta oportunidades para desarrollar industrias creativas y tecnológicas. El proyecto es pertinente para diversificar la economía local y generar nuevas oportunidades de empleo.',
    'Santiago': 'Santiago, como centro económico y cultural del país, presenta múltiples oportunidades para iniciativas innovadoras. El proyecto es pertinente para aprovechar la concentración de talento, recursos y mercado disponible.',
  };

  return pertinencias[sede] || `El proyecto es pertinente para ${sede} al abordar necesidades específicas de la región en el área ${focalizacion || 'correspondiente'}.`;
}

function getPertinenciaDisciplinar(nombreProyecto: string, sede: string): string {
...
function getSolucionAvance(nombreProyecto: string, avance: number): string {
  const avanceTexto = avance > 0 ? `El proyecto se encuentra en un ${avance}% de avance, con actividades fundamentales ya implementadas.` : 'El proyecto se encuentra en fase inicial de implementación.';
...
function getEjesImpacto(focalizacion: string | null, nombreProyecto: string): string {
  const ejes = [];
  
  if (focalizacion === 'Social') {
    ejes.push('Impacto Social: Fortalecimiento de la cohesión social, mejora de la calidad de vida de la población objetivo, reducción de desigualdades, y promoción de la inclusión y diversidad cultural.');
  }
  
  if (focalizacion === 'Productiva') {
    ejes.push('Impacto Productivo: Generación de empleo, desarrollo de nuevas industrias, fortalecimiento del ecosistema empresarial local, y diversificación de la economía regional.');
  }
  
  if (focalizacion === 'Ambiental') {
    ejes.push('Impacto Ambiental: Reducción del impacto ambiental, promoción de prácticas sostenibles, conservación de recursos naturales, y educación sobre sostenibilidad.');
  }
  
  // Agregar impacto tecnológico si es relevante
  if (nombreProyecto.includes('TechLakou') || nombreProyecto.includes('AntofaSuena')) {
    ejes.push('Impacto Tecnológico: Reducción de brechas digitales, modernización de procesos, adopción de nuevas tecnologías, y fortalecimiento de competencias digitales en la población.');
  }
  
  if (ejes.length === 0) {
    ejes.push('Impacto Integral: El proyecto genera impacto en múltiples dimensiones, contribuyendo al desarrollo integral de la región mediante la formación de capital humano, la innovación, y el fortalecimiento de las capacidades locales.');
  }
