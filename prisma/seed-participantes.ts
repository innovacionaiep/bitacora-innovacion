...
function generarNombre(index: number, tipo: 'colaborador' | 'docente' | 'estudiante' | 'beneficiario'): string {
  const nombres = {
    colaborador: ['Juan Pérez', 'María González', 'Carlos Rodríguez', 'Ana Martínez', 'Luis Sánchez'],
    docente: ['Prof. Patricia López', 'Prof. Roberto Díaz', 'Prof. Carmen Ruiz', 'Prof. Fernando Torres', 'Prof. Laura Vega'],
    estudiante: [
...
function generarEmail(proyectoId: string, index: number, tipo: 'colaborador' | 'docente' | 'estudiante' | 'beneficiario'): string {
...
async function asignarParticipantes() {
  try {
    ...
    for (const proyecto of proyectos) {
      ...
      const participantes = [...colaboradores, ...docentes, ...estudiantes, ...beneficiarios];
      ...
      console.log(`  ✅ Asignados: 5 Colaboradores, 5 Docentes, 15 Estudiantes, 10 Beneficiarios`);
    }
