  ...
  try {
    ...
    for (const proyecto of proyectos) {
      ...
      if (todasLasFechas.length === 0) {
        console.log(`⚠️  Proyecto ${proyecto.proyecto} no tiene tareas con fechas. Usando fechas por defecto.`);
        ...
        for (const indicador of proyecto.indicadores) {
          if (!indicador.fechaInicio || !indicador.fechaFin) {
            ...
            const diasProyecto = Math.floor((fechaFinProyecto.getTime() - fechaInicioProyecto.getTime()) / (1000 * 60 * 60 * 24));
            const diasInicio = Math.floor(Math.random() * (diasProyecto * 0.7)); // Inicio en los primeros 70% del proyecto
            const duracionIndicador = Math.floor(Math.random() * (diasProyecto * 0.3)) + 30; // Duración entre 30 días y 30% del proyecto
      ...
      if (fechasOrdenadas.length === 0) {
        console.log(`⚠️  Proyecto ${proyecto.proyecto} no tiene fechas válidas.`);
        continue;
      }
      ...
      if (diasProyecto <= 0) {
        console.log(`⚠️  Proyecto ${proyecto.proyecto} tiene rango de fechas inválido.`);
        continue;
      }
      ...
      for (const indicador of proyecto.indicadores) {
        if (!indicador.fechaInicio || !indicador.fechaFin) {
          ...
          const duracionIndicador = Math.floor(Math.random() * (diasProyecto * 0.4)) + 30;
      ...
      console.log(`✅ Proyecto ${proyecto.proyecto}: ${proyecto.indicadores.length} indicadores procesados`);
    }
